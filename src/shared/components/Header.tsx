...
export function Header() {
	...
	return (
		<>
			<header className='mix-blend-exclusion z-50 w-full fixed top-0 [&>.xxx]:z-60 [&>.xxx]:relative '>
			...
			<div
				...
				<ul className='flex h-[45px] items-center justify-between border  border-white'>
					{[
						{ mob: 'ig', default: 'instagram' },
						{ mob: 'tw', default: 'twitter' },
						{ mob: 'fb', default: 'facebook' },
						{ mob: 'tg', default: 'youtube' },
					].map((item, index) => (
						<React.Fragment key={index}>
							<li className='flex-[1 0 0px] px-5 w-full flex justify-center'>
								<Button variant={'ghost'}>
									<span className='hidden md:block'>{item.default}</span>
